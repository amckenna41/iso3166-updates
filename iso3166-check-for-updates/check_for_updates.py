...
#bool that tracks if ISO3166 updates have been found
...
def check_iso3166_updates_main(request):
    ...
    latest_iso3166_updates = iso3166_updates.get_updates(alpha2_codes, export_json_filename="new-iso3166-updates", 
    ...
    def update_json(latest_iso3166_updates_after_date_filter):
        ...
        if not (blob.exists()):
            raise ValueError("Error retrieving updates data json.")
        
        #download current ISO3166 updates JSON file from storage bucket 
    ...
    def create_issue(latest_iso3166_updates_after_date_filter, month_range):
        ...
        for code in list(latest_iso3166_updates_after_date_filter.keys()):
            ...
            temp = json.dumps(latest_iso3166_updates_after_date_filter[code])
