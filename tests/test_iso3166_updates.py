...
class ISO3166_Updates_Tests(unittest.TestCase):
    ...
    def test_iso3166_updates_metadata(self): 
        ...
        # self.assertEqual(metadata('iso3166_updates')['version'], "1.3.0.", 
        ...
        # self.assertEqual(metadata('iso3166_updates')['keywords'], "iso,iso3166,beautifulsoup,python,pypi,countries,country codes,csv,iso3166-2,iso3166-1,alpha-2,alpha-3", 
    ...
    def test_wiki_url(self):
        """ Test each ISO 3166-2 wiki URL endpoint to check valid status code 200 is returned. """
        #get list of alpha-2 codes from iso3166 library
        alpha2_codes = list(iso3166.countries_by_alpha2.keys())

        #iterate over each ISO 3166 alpha-2 code, testing response code using request library
...
#7.)
        with self.assertRaises(TypeError):
           test_table = iso3166_updates.table_to_array(test_alpha2_error_1)
           test_table = iso3166_updates.table_to_array(test_alpha2_error_2)
           test_table = iso3166_updates.table_to_array(test_alpha2_error_3)

    def test_get_updates_df(self):
        """ Test function that converts 2D parsed html table into dataframe. """
...
#1.)
        soup = BeautifulSoup(requests.get(self.wiki_base_url + test_alpha2_az).content, "html.parser") #Azerbaijan
        table_html = soup.find("span", {"id": "Changes"}).findNext('table')
        ...
        self.assertEqual(len(az_updates_df), 2, "Expected there to be 2 elements in output dataframe, got {}.".format(len(az_updates_df)))
        self.assertEqual(self.expected_output_columns, list(az_updates_df.columns), "Columns/Headers of dataframe do not match:\n{}".format(list(az_updates_df.columns)))
...
#2.)
        soup = BeautifulSoup(requests.get(self.wiki_base_url + test_alpha2_fi).content, "html.parser") #Finland
        table_html = soup.find("span", {"id": "Changes"}).findNext('table')
        ...
        fi_expected_output1 = ['2011-12-15', 'Newsletter II-3[1]', 'Subdivision layout: 6 provinces (see below) -> 19 regions',
        ...
        fi_expected_output2 = ['2022-11-29', 'ISO Online Browsing Platform (OBP) \
            (https://www.iso.org/obp/ui/#iso:code:3166:FI)', 'Name change: Satakunda -> Satakunta',
            'Change of spelling of FI-17; Update List Source']
        ...
        self.assertEqual(self.expected_output_columns, list(fi_updates_df.columns), "Columns/Headers of dataframe do not match:\n{}".format(list(fi_updates_df.columns)))
...
#3.)
        soup = BeautifulSoup(requests.get(self.wiki_base_url + test_alpha2_gh).content, "html.parser") #Ghana
        table_html = soup.find("span", {"id": "Changes"}).findNext('table')
        ...
        self.assertEqual(self.expected_output_columns, list(gh_updates_df.columns), "Columns/Headers of dataframe do not match:\n{}".format(list(gh_updates_df.columns)))
...
#4.)
        soup = BeautifulSoup(requests.get(self.wiki_base_url + test_alpha2_ke).content, "html.parser") #Kenya
        table_html = soup.find("span", {"id": "Changes"}).findNext('table')
        ...
        self.assertEqual(self.expected_output_columns, list(ke_updates_df.columns), "Columns/Headers of dataframe do not match:\n{}".format(list(ke_updates_df.columns)))
...
#5.)
        soup = BeautifulSoup(requests.get(self.wiki_base_url + test_alpha2_sn).content, "html.parser") #Senegal
        table_html = soup.find("span", {"id": "Changes"}).findNext('table')
        ...
        self.assertEqual(self.expected_output_columns, list(sn_updates_df.columns), "Columns/Headers of dataframe do not match:\n{}".format(list(sn_updates_df.columns)))
...
#6.)
        with self.assertRaises(TypeError):
            test_updates_df = iso3166_updates.get_updates_df(test_alpha2_error_1)
            test_updates_df = iso3166_updates.get_updates_df(test_alpha2_error_2)
            test_updates_df = iso3166_updates.get_updates_df(test_alpha2_error_3)
...
#1.) 
        test_alpha2_au_updates = iso3166_updates.get_updates(alpha2_codes=test_alpha2_au,   #Australia
            export_filename=self.export_filename, export_folder=self.export_folder, 
                concat_updates=True, export_json=True, export_csv=True, verbose=0)
...
#3.)
        test_alpha2_id_updates = iso3166_updates.get_updates(test_alpha2_id,    #Indonesia
            export_filename=self.export_filename, export_folder=self.export_folder, 
                export_json=True, export_csv=True, verbose=0)
...
#4.)
        test_alpha2_pt_updates = iso3166_updates.get_updates(test_alpha2_pt, export_filename=self.export_filename, #Portugal
            export_folder=self.export_folder, export_json=True, export_csv=True, verbose=0)
...
#5.)
        test_alpha2_bf_ca_gu_ie_je_updates = iso3166_updates.get_updates(test_alpha2_bf_ca_gu_ie_je_str, #concat_updates=False, individual output files
            export_filename=self.export_filename, export_folder=self.export_folder, 
                concat_updates=False, export_json=True, export_csv=True, verbose=0)
...
#6.)    
        test_alpha2_bf_ca_gu_ie_je_updates = iso3166_updates.get_updates(test_alpha2_bf_ca_gu_ie_je_str, #concat_updates=True, one output file
            export_filename=self.export_filename, export_folder=self.export_folder, 
                concat_updates=True, export_json=True, export_csv=True, verbose=0) 
...
#8.)
    ...
    def test_get_updates_year(self):
        ...
        test_year9 = True
#1.)    
        test_year_2017_updates = iso3166_updates.get_updates(year=test_year1, export_filename=self.export_filename, 
                export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
...
#2.)
        test_year_2005_updates = iso3166_updates.get_updates(year=test_year2, export_filename=self.export_filename, 
                export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
...
#3.)
        test_year_gt_2021_updates = iso3166_updates.get_updates(year=test_year3, export_filename=self.export_filename, 
                export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
        test_year_gt_2021_expected_keys = ['CN', 'DZ', 'ET', 'FI', 'FR', 'GB', 'GT', 'HU', 'ID', 'IQ', 'IS', 'KH', 'KP', \
                                           'KZ', 'LT', 'LV', 'NP', 'NZ', 'PA', 'RU', 'SI', 'SS']
        ...
        self.assertTrue(os.path.isfile(os.path.join(self.export_folder, self.export_filename + "_>2021.json")),
            "Expected output JSON file to exist in folder.")
        self.assertEqual(len(test_year_gt_2021_updates), 22, 
            "Expected 22 updates in output object, got {}.".format(len(test_year_gt_2021_updates)))
        ...
        self.assertEqual(len(test_gt_2021_iso3166_csv), 28, 
            "Expected there to be 28 output objects in csv, got {}.".format(len(test_gt_2021_iso3166_csv)))
#4.)
        test_year_lt_2003_updates = iso3166_updates.get_updates(year=test_year4, export_filename=self.export_filename, 
                export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
        ...
        self.assertTrue(os.path.isfile(os.path.join(self.export_folder, self.export_filename + "_<2003.json")),
            "Expected output JSON file to exist in folder.")
        self.assertEqual(len(test_year_lt_2003_updates), 58, 
            "Expected 58 updates in output object, got {}.".format(len(test_year_lt_2003_updates)))
        ...
        self.assertEqual(len(test_lt_2003_iso3166_csv), 78, 
            "Expected there to be 78 output objects in csv, got {}.".format(len(test_lt_2003_iso3166_csv)))
#5.)
        test_year_2005_2007_updates = iso3166_updates.get_updates(year=test_year5, export_filename=self.export_filename, 
                export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
...
#6.)
        test_year_1999_2002_updates = iso3166_updates.get_updates(year=test_year6, export_filename=self.export_filename, 
                export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
...
#7.)
        ...
        self.assertTrue(os.path.isfile(os.path.join(self.export_folder, self.export_filename + ".json")),
            "Expected output JSON file to exist in folder.")
        self.assertEqual(len(test_year_error1_updates), 250, 
        ...
        self.assertEqual(len(test_year_error1_updates_csv), 585, 
            "Expected there to be 585 output objects in csv, got {}.".format(len(test_year_error1_updates_csv)))
#8.)
        with self.assertRaises(TypeError):
            test_year_error2_updates = iso3166_updates.get_updates(year=test_year8, export_filename=self.export_filename, 
                    export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
            test_year_error3_updates = iso3166_updates.get_updates(year=test_year9, export_filename=self.export_filename, 
                    export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=True, verbose=0)
           
    def test_get_updates_alpha2_year(self):
        """ Testing main updates function that gets the updates and exports to json/csv, using
            a variety of alpha-2 codes and year input parameter values. """
        test_ch_2003 = ("CH", "2003")
        test_md_2008 = ("MD", "2008")
        test_ne_2010 = ("NE", "2010") #{}
        test_sd_tt_2015_2021 = ("SDN,TTO", "2015-2021") #using alpha-3 codes
        test_es_gq_kg_sm_2020 = ("es,gq,kg,sm", "2020")
        test_ie_2027 = ("IE", "2027")
        test_error1 = ("ABC", "3003")
        test_error2 = ("ABCDEF", "ABC")
#1.) 
        test_alpha2_year_ch_2003_updates = iso3166_updates.get_updates(alpha2_codes=test_ch_2003[0], year=test_ch_2003[1], 
                export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, \
                    export_csv=False, verbose=0)
...
#2.)
        test_alpha2_year_md_2008_updates = iso3166_updates.get_updates(alpha2_codes=test_md_2008[0], year=test_md_2008[1], 
                export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, \
                    export_csv=False, verbose=0)
...
#3.) 
        test_alpha2_year_ne_2010_updates = iso3166_updates.get_updates(alpha2_codes=test_ne_2010[0], year=test_ne_2010[1], 
                export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, \
                    export_csv=False, verbose=0)
...
#4.)
        test_alpha2_year_sd_tt_2015_2021_updates = iso3166_updates.get_updates(alpha2_codes=test_sd_tt_2015_2021[0], year=test_sd_tt_2015_2021[1], 
...
#5.)
        test_alpha2_year_es_gq_kg_sm_2020_updates = iso3166_updates.get_updates(alpha2_codes=test_es_gq_kg_sm_2020[0], year=test_es_gq_kg_sm_2020[1], 
                export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=False, verbose=0)
...
#6.) 
        test_alpha2_year_ie_2027_updates = iso3166_updates.get_updates(alpha2_codes=test_ie_2027[0], year=test_ie_2027[1], 
                export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=False, verbose=0)
...
#7.)
        with self.assertRaises(ValueError):
            test_error1_updates = iso3166_updates.get_updates(alpha2_codes=test_error1[0], year=test_error1[1], 
                    export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=False, verbose=0)
            test_error2_updates = iso3166_updates.get_updates(alpha2_codes=test_error2[0], year=test_error2[1], 
                    export_filename=self.export_filename, export_folder=self.export_folder, concat_updates=True, export_json=True, export_csv=False, verbose=0)
...
#1.)
        #iterate over each ISO3166 alpha-2 code, testing alpha-2 code is in updates json
        for code in alpha2_codes:
...
#6.)
        self.assertEqual(self.iso3166_json[test_alpha2_pm], {}, "Expected output to be an empty dict, got {}.".format(self.iso3166_json[test_alpha2_pm]))
#7.)
        self.assertEqual(self.iso3166_json[test_alpha2_pn], {}, "Expected output to be an empty dict, got {}.".format(self.iso3166_json[test_alpha2_pn]))
#8.)   
